...
- **Chinese and English interface**: switch the app language in Settings → General → App Language. The default is Simplified Chinese; your app language is independent of the languages you translate.
...
1. In Settings → General, choose English under App Language if you want the English interface. Select a translation provider and target language separately. Apple Translation needs no endpoint configuration. For an LLM, click Add Service, enter a service name, Base URL, API Key, and model name, then click Save. Use a base address such as `http://localhost:1234/v1`, without `/chat/completions`. Start local model servers before using them.
2. In Settings → Shortcuts, grant the permissions you need: Accessibility for selected-text translation and Screen Recording for screenshot translation.
3. Choose “Start for 3 selections” from the menu bar, then select text in another app and click the Translate button above the selection. To change the default mode, count, or duration, open Settings → Sessions, then use the shortcut to start a session.

Screenshot translation can be started directly from the menu bar without an active translation session.

Save multiple LLM services and switch between them in settings or the results panel. Service edits require an explicit Save; switching providers preserves drafts within the settings window, and closing settings discards unsaved edits. Apple Translation cannot be deleted. Upgrades select Apple by default and preserve the old endpoint as “Previous service”. Selected text, clipboard, and screenshots all use the current provider. Apple screenshot translation uses local Vision OCR. With an LLM provider, screenshots are sent to the selected service even when on-device Vision OCR is selected.
