...
1. 在「设置 → 通用」选择翻译来源和目标语言。默认的「Apple 翻译」无需接口配置。使用 LLM 时，点击「添加服务」，填写服务名称、接口地址（Base URL）、API Key 和模型名称，再点击「保存」。接口地址例如 `http://localhost:1234/v1`，不要包含 `/chat/completions`；使用本地模型前，先启动对应服务。
