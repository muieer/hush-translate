...
actor TranslateService {
    ...
    enum TranslateError: LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .invalidURL(let s): return L10n.format("URL 不合法: %@", s)
            case .http(let code, let body): return "HTTP \(code): \(body.prefix(200))"
            case .emptyResponse: return L10n.tr("模型返回为空")
            case .decoding(let s): return L10n.format("响应解析失败: %@", s)
        ...
        if let imgData = req.imageData {
            ...
            if case .screenshot = req.source {
                if L10n.language == .english {
                    userText = "Read all text in the image in its original order, preserving paragraphs and line breaks, then translate it into \(displayName(req.targetLang)). Output the original text first, followed by a blank line and the translation."
                } else {
    ...
    private func systemPrompt(for req: TranslationRequest, config: TranslateConfig) -> String {
        let src = req.sourceLang == "auto" ? L10n.tr("源语言（自动检测）") : displayName(req.sourceLang)
